...
describe("resume-portfolio hash", () => {
  it("buildPortfolioCtaHref: caseId와 sectionId를 CTA 링크 규칙으로 생성한다", () => {
    expect(buildPortfolioCtaHref("exem-data-grid", "threads")).toBe(
      "/portfolio/exem-data-grid#threads"
    )
  })

  it("buildPortfolioPath: 하위 호환 path 규칙을 생성한다", () => {
    expect(buildPortfolioPath("exem-data-grid", "threads")).toBe("/exem-data-grid#threads")
  })

  it("buildPortfolioCtaHref: 유효하지 않은 caseId면 에러를 던진다", () => {
    expect(() => buildPortfolioCtaHref("Exem Data Grid", "hook")).toThrow("Invalid caseId format")
  })

  it("parsePortfolioCtaHref: 유효한 CTA 링크를 파싱한다", () => {
    expect(parsePortfolioCtaHref("/portfolio/exem-customer-dashboard#threads")).toEqual({
      caseId: "exem-customer-dashboard",
      sectionId: "threads",
    })
  })

  it("parsePortfolioCtaHref: 레거시 section 해시는 canonical section으로 정규화한다", () => {
    expect(parsePortfolioCtaHref("/portfolio/exem-customer-dashboard#overview")).toEqual({
      caseId: "exem-customer-dashboard",
      sectionId: "hook",
    })
    expect(parsePortfolioCtaHref("/portfolio/exem-customer-dashboard#problem")).toEqual({
      caseId: "exem-customer-dashboard",
      sectionId: "threads",
    })
    expect(parsePortfolioCtaHref("/portfolio/exem-customer-dashboard#decision")).toEqual({
      caseId: "exem-customer-dashboard",
      sectionId: "threads",
    })
  })
  ...
  it("parsePortfolioPath: 유효한 해시를 파싱한다", () => {
    expect(parsePortfolioPath("/exem-customer-dashboard#threads")).toEqual({
      caseId: "exem-customer-dashboard",
      sectionId: "threads",
    })
  })
  ...
  it("parsePortfolioPath: / 없이 들어와도 파싱한다", () => {
    expect(parsePortfolioPath("exem-new-generation#context")).toEqual({
      caseId: "exem-new-generation",
      sectionId: "context",
    })
  })

  it("parsePortfolioPath: 레거시 section 해시는 canonical section으로 정규화한다", () => {
    expect(parsePortfolioPath("/exem-data-grid#result")).toEqual({
  ...
  it("parsePortfolioPath: 허용되지 않은 caseId 포맷이면 null을 반환한다", () => {
    expect(parsePortfolioPath("/exem_data_grid#threads")).toBeNull()
  })
})
