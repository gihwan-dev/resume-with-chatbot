...
export const RESUME_PORTFOLIO_CONTENT_V2: ResumePortfolioContentItem[] = [
  ...
  },
  {
    ...
    resumeSummary:
      "대규모 데이터 조회와 빈번한 열 조작이 동시에 일어나는 화면을 위해, **20+ 기능을 지원하는 공용 데이터 그리드**를 재설계했습니다. 기존 table 기반 구조를 div 기반 렌더링 구조로 전환하고, virtualization·리사이즈·고정 컬럼·그룹 헤더를 안정적으로 조합할 수 있는 기반을 만들었습니다.",
    accomplishments: [
      "기존 table 태그 구조가 가상화와 유연한 레이아웃 구현에 한계가 있어, 통합 테스트를 먼저 작성한 뒤 div 기반 구조로 리팩토링했습니다.",
      "초기에는 셀 단위 absolute positioning으로 렌더링했지만, 칼럼 너비 조정 시 인접 칼럼이 함께 재분배되는 스펙 변경에 맞춰 **Row는 absolute, Cell은 flex 기반**으로 다시 설계했습니다.",
      "빈 공간 없는 리사이즈, 좌·우 고정 컬럼의 반대 방향 영향 범위, 리프 헤더만 리사이즈 가능, 그룹 헤더 너비는 자식 너비의 합이라는 제약 조건을 규칙으로 정리해 복잡한 동작을 일관되게 구현했습니다.",
      "컨테이너를 상·중·하 / 좌·중·우 영역으로 분리하고, 제어·비제어를 모두 지원하는 일관된 인터페이스를 설계해 다양한 화면 요구사항에 대응했습니다.",
      "[TanStack Table Custom Features](https://tanstack.com/table/latest/docs/guide/custom-features) 기반으로 **Core - State - UI 계층**을 분리해 기능 확장 시 변경 범위를 줄였고, 이후 다른 프레임워크 기반 테이블에도 적용 가능한 구조로 발전시켰습니다.",
