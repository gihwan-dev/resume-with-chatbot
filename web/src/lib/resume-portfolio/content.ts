...
export const RESUME_PORTFOLIO_CONTENT_V2: ResumePortfolioContentItem[] = [
  ...
  },
  {
    ...
    resumeSummary:
      "공용 데이터 그리드를 기존 `table` 기반 구조에서 `div` 기반 렌더링으로 재설계해, 고정 컬럼·그룹 헤더·대규모 데이터 조합에서 반복되던 레이아웃 제약을 해소했습니다. 이후 Row absolute + Cell flex 구조로 DOM 구조를 단순화했고, 테이블 루트 이벤트 위임과 행 단위 메모이제이션을 각각 적용해 리사이즈 처리 시간을 `22ms → 0.5ms`로 줄이고 이벤트 리스너 수를 `731 → 343`으로 낮췄습니다.",
    accomplishments: [
      "기존 `table` 기반 구현은 가상화, 고정 컬럼, 그룹 헤더 조합에서 제약이 커서, ARIA role을 유지한 `div` 기반 렌더링으로 전환했습니다.",
      "Cell absolute 중심 구조를 Row absolute + Cell flex 구조로 재설계해 DOM과 레이아웃 계산 경로를 단순화했고, 컨테이너 리사이즈 처리 시간을 `22ms → 0.5ms`로 줄였습니다.",
      "테이블 루트 이벤트 위임으로 셀·행마다 생성되던 클릭 핸들러를 루트 2개 핸들러로 수렴시켜 이벤트 리스너를 `731 → 343`, 힙 사용량을 `138MB → 104MB`로 낮췄습니다.",
      "스크롤 시 전체 셀이 다시 그려지는 원인을 추적해 행 단위 `React.memo`와 `areTableRowPropsEqual` comparator를 적용하고 `columnSizing` 참조를 안정화했습니다. 스크롤 구간 리렌더를 보존했고, 최대 CPU는 `58% → 28%`, 힙은 `138MB → 98MB`까지 낮췄습니다.",
      "[TanStack Table Custom Features](https://tanstack.com/table/v8/docs/guide/custom-features) + bridge 패턴으로 기능을 모듈화하고, 기능 호환표를 기준으로 Storybook Portable Stories와 Vitest Browser 테스트를 운영해 핵심 조합 회귀를 관리했습니다.",
    ],
    hasPortfolio: true,
    ctaLabel: "상세 케이스 스터디 보기",
    defaultSectionId: "tldr",
