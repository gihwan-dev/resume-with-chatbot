...
- Table 9분할 구조에서 컬럼 리사이즈 정책을 `우측 캐스케이드 + Flex Basis`로 전환하고, 고정행/스크롤 동기화 이슈를 스토리·브라우저 테스트로 재현-수정해 회귀 리스크를 낮추고 있습니다.
...
- 차세대 대시보드 위젯 빌더 컴포넌트 분리 및 재사용성 개선 — Table/Scatter 위젯 간 공통 컴포넌트 공유 구조 설계
...
- AI 기반 내부 개발 도구(ai-essence) CLI 개발 및 배포 파이프라인 구축
