  ...
  threads:
    - issueTitle: "단일 차트 폼 스키마 집중으로 확장 시 복잡도가 급증하는 문제"
      ...
      comparison:
        ...
        before:
          - "5종 차트 설정이 단일 스키마에 집중되어 조건 분기가 빠르게 증가했습니다."
          - "새 차트 추가 시 공통 스키마 수정이 반복되어 변경 영향 범위가 컸습니다."
          - "버그 원인 추적 시 UI/스키마 결합 지점을 동시에 파악해야 했습니다."
        after:
          - "차트 타입별 독립 스키마·UI 구성으로 변경 범위를 도메인 단위로 제한했습니다."
          - "공통 입력 요소를 빌딩 블록으로 재사용해 확장 비용을 줄였습니다."
          - "검증 규칙/어댑터 계층으로 차트 추가 시 중앙 병목 없이 증분 확장이 가능해졌습니다."
