...
storyThread:
  ...
  implementationGroups:
    ...
    - title: "React 단계"
      items:
        - "API 전면 교체 시점에 React와 사내 디자인 시스템으로 대시보드를 재구축하면서, DockingFrame↔스토어 강결합을 ERD 기반 Entity 재설계(WebEnv 영구저장·BufferMemory 메모리저장)로 해소했습니다."
        - "위젯별 독립 API 호출 + React Query 폴링 구조로 전환해 대시보드 상태 관리를 위젯 단위로 격리했습니다."
