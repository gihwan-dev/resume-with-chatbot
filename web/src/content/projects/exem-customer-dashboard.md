...
priority: 1
storyThread:
  tldrSummary: "300~3,000대 규모 인스턴스를 운영하는 실시간 대시보드와 그룹 단위 일·월·연 추세 분석 제품 2종을 개발하면서, Vue2 레거시의 폴링/영속 상태 구조를 먼저 정리하고 API 전면 교체 시점에 React 재구축으로 전환했습니다. 허브 제품에 맞춰 고밀도 그리드와 1단계 이동 동선을 적용하고 알림 기능은 테스트로 스펙을 고정한 뒤 마이그레이션했으며, 인터랙션 중 상태 반영 유예와 렌더 우선순위 조정으로 이상 징후 인지 시간을 10초에서 3초로 단축하고 조작 지연을 73~82% 개선했습니다."
  keyMetrics:
    - value: "10초 -> 3초"
    ...
    - value: "2종"
      label: "운영 제품 개발"
      description: "실시간 상태 모니터링 대시보드와 그룹 단위 추세 분석 제품을 함께 구축했습니다."
  coreApproach: "Vue 레거시 구조 정리와 API 전면 교체 시점의 React 재구축을 분리하지 않고, 허브 UX 개선·알림 마이그레이션 안정화·인터랙션 최적화를 하나의 제품 개선 흐름으로 설계했습니다."
  ...
  problemPoints:
    ...
    - "Vue2 레거시 화면마다 폴링 규칙이 흩어져 있어 운영 재현성이 낮았습니다."
    - "로컬 스토리지에 연관된 필터/조회 상태가 분산 저장돼 변경 영향 범위를 예측하기 어려웠습니다."
    - "API 전면 교체가 예정된 상황에서 전역 객체 의존이 커서 기능 추가보다 이후 마이그레이션 비용이 더 커졌습니다."
    - "허브 제품임에도 다른 제품으로 이동하는 연계 동선이 깊어 빠른 대응 흐름이 끊겼습니다."
    - "알림 기능은 스펙이 복잡해 마이그레이션 전에 동작을 먼저 고정할 필요가 있었습니다."
    - "3초 주기 폴링과 리사이즈·드래그 인터랙션이 겹칠 때 조작 지연이 발생했습니다."
  decisions:
    - title: "Vue 단계에서 폴링을 객체로 추상화"
      whyThisChoice: "화면마다 따로 놀던 타이머를 유지하면 운영 시나리오 재현과 유지보수 비용이 계속 증가했습니다."
      alternative: "화면별 타이머 로직을 유지하는 대신 restart/add/stop 메서드 중심 객체 추상화로 통합"
      tradeOff: "초기 구조 설계 부담은 늘지만 갱신 주기를 선언적으로 제어해 운영 일관성과 확장성을 확보할 수 있어 객체 추상화를 선택했습니다."
    - title: "영속 상태를 중앙화하고 버전 관리 구조로 개편"
      whyThisChoice: "필터/조회 상태가 여러 위치에 분산되면 변경 영향 범위를 예측하기 어렵고 회귀 위험이 커졌습니다."
      alternative: "기존 분산 저장을 유지하는 대신 연관 상태를 중앙화하고 버전 기반 마이그레이션 구조 적용"
    ...
    - title: "Vue 연장 대신 React + 사내 디자인 시스템 재구축"
      ...
      alternative: "Vue 구조를 추가 확장하는 대신 React와 사내 디자인 시스템 기반으로 재구축"
    ...
    - title: "허브 UX는 정보 밀도와 이동 깊이, 인터랙션 안정성을 우선"
      ...
      alternative: "카드형 UI와 깊은 이동 동선을 유지하는 대신 고밀도 그리드·1단계 진입·상태 반영 유예 적용"
  ...
  implementationGroups:
    - title: "Vue 단계"
      items:
        - "화면마다 분산된 타이머 폴링을 restart/add/stop 메서드 중심 객체로 추상화했습니다."
        - "로컬 스토리지 필터/조회 상태를 중앙화하고 버전 기반 구조로 관리했습니다."
        - "JSDoc 타입 시스템을 도입해 레거시 코드의 변경 예측 가능성과 리팩토링 안정성을 높였습니다."
        - "실시간 지표 화면, 추세 상세 화면, Top 인스턴스 비교 화면을 운영 시나리오에 맞춰 확장했습니다."
    - title: "React 단계"
      items:
  ...
  validationImpact:
    ...
    metrics:
      - "장애 인지 시간: 10초 -> 3초"
      - "인터랙션 지연: 73~82% 감소"
    operationalImpact: "운영자는 한 화면에서 비교·판단·이동·대응 흐름을 끊지 않고 처리할 수 있게 됐고, 팀은 복잡 기능 마이그레이션의 품질 기준을 테스트로 고정할 수 있게 됐습니다."
...
300~3,000대 규모 인스턴스를 운영하는 실시간 대시보드와 그룹 단위 일·월·연 추세 분석 제품 2종을 개발하면서, Vue 레거시의 폴링/영속 상태를 먼저 정리하고 API 전면 교체 시점에 React로 재구축했습니다. 허브 UX를 고밀도 그리드와 1단계 이동으로 바꾸고 알림 기능은 테스트 선행 마이그레이션으로 안정화했으며, 인터랙션 중 상태 반영 유예와 렌더 우선순위 조정으로 인지 시간 10초→3초, 조작 지연 73~82% 개선을 만들었습니다.
