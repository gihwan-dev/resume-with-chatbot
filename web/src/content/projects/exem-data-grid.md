...
* `tr`/`th` 같은 경직된 요소 계층 구조로 인해 리사이즈 핸들러를 헤더와 동일 계층에 배치하는 등 유연한 렌더링 구성이 어려웠습니다.
* 셀 위치 계산이 `column.getStart()` 누적 합산에 의존해 열 수에 비례한 `O(n)` 비용이 발생했고, 컨테이너 리사이즈 처리에도 오버헤드가 발생했됐습니다.
