...
* `tr`/`th` 같은 경직된 요소 계층 구조로 인해 리사이즈 핸들러를 헤더와 동일 계층에 배치하기 어렵고, 다단 그룹 헤더에서 히트박스 단절로 리사이즈 실패가 반복됐습니다.
...
* 고정 행/열을 3×3 패널로 분리하는 구조를 설계하고, `TanStack Virtual`(`useVirtualizer`)로 행 가상화를 적용해 Row `position: absolute` / Cell `display: flex` 구조로 렌더링 경계를 재구성했습니다.
* 셀 배치에서 `left`·`width` 직접 계산과 `column.getStart()` 누적 합산을 제거하고, `flex` 기반 폭 정책(고정폭/자동폭)으로 전환해 레이아웃 계산을 브라우저 엔진에 위임했습니다.
* 다단 그룹 헤더의 리사이즈 히트박스 단절을 해소하기 위해 리사이즈 계산을 UI 이벤트 처리와 정책 모듈로 분리하고, 헤더 메타 컨텍스트 계층을 도입했습니다.
* `flex` 전환으로 컬럼 폭 보정이 불필요해져 `ResizeObserver` 기반 보정 경로를 제거했습니다.
